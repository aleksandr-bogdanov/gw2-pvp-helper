/**
 * Browser-side anchor detection via NCC template matching.
 *
 * Pure math — identical to server version.
 * Templates loaded via fetch + Canvas instead of fs + Sharp.
 */
import { loadTemplateFromUrl, extractROI } from './preprocess.js';
...
async function loadTemplates(): Promise<Template[]> {
	...
	for (const { file, uiSize } of templateFiles) {
		try {
			const image = await loadTemplateFromUrl(`/scan-data/templates/${file}`);
...
function teamColorScore(
	...
	for (let i = 0; i < 5; i++) {
		...
		if (
			rp.x - half >= 0 && rp.x + half < rgbImage.width &&
			rp.y - half >= 0 && rp.y + half < rgbImage.height
		) {
			let rSum = 0, bSum = 0;
		...
		if (
			bp.x - half >= 0 && bp.x + half < rgbImage.width &&
			bp.y - half >= 0 && bp.y + half < rgbImage.height
		) {
			let rSum = 0, bSum = 0;
			const ss = Math.min(half, 10);
			for (let dy = -ss; dy <= ss; dy += 2) {
 ...
 * Identical logic to server version.
 */
