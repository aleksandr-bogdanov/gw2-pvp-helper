<script lang="ts">
	...
	import { themes, theme, cycleTheme } from '$lib/theme.svelte.js';
...
<div class="min-h-screen bg-(--color-bg)">
	...
	<nav aria-label="Main navigation" class="glass sticky top-0 z-40 border-b border-(--color-border) shadow-sm">
		<div class="mx-auto max-w-3xl flex items-center justify-between px-6 py-3">
			<a href="/app" class="text-base font-semibold tracking-tight text-(--color-text) hover:text-(--color-accent) transition-colors">
				GW2 PvP Helper
			</a>
			<div class="flex items-center gap-4">
				...
				<button
					...
					title="Cycle theme: {themes.find((t) => t.id === theme.current)?.label}"
				>
					{themes.find((t) => t.id === theme.current)?.label}
				</button>
